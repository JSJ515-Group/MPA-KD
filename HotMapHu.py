...
class yolo_heatmap:
    ...
    def process(self, img_path, save_path):
        ...
        cam_image = show_cam_on_image(img, grayscale_cam, use_rgb=True)  # 这是以前的，
...
def get_params():
    params = {
        'weight': r'E:\BaiduNetdiskDownload\change-ASFF-Rep\runs\train\exp88\weights\best.pt',
        'device': 'cuda:0',
        'method': 'EigenCAM',
        'layer': [36, 37, 38],
        'backward_type': 'class',
...
if __name__ == '__main__':
    ...
    weight_path = os.path.join(current_dir, 'runs', 'train', 'exp88', 'weights', 'best.pt')
    test_img_path = os.path.join(current_dir, 'runs', 'train', 'exp88', 'weights', 'test7.jpg')
    save_dir = os.path.join(current_dir, 'runs', 'train', 'exp88', 'heatmap_results7')

    print("=== 调试信息 ===")
    print(f"当前目录: {current_dir}")
    print(f"权重文件: {'存在' if os.path.exists(weight_path) else '不存在'}")
    print(f"测试图片: {'存在' if os.path.exists(test_img_path) else '不存在'}")
    ...
    params = get_params()
    params['weight'] = weight_path

    print("开始初始化模型...")
    model = yolo_heatmap(**params)
    print("模型初始化完成，开始生成热力图...")

    model(test_img_path, save_dir)
    print("处理完成！")